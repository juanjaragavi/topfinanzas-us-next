...
export async function POST(req: Request) {
  ...
  try {
    ...
    apiLogger.debug("Environment check (short-version)", {
    ...
    apiLogger.debug("Google Auth created successfully (short-version)");
    ...
    if (sheetValues.length === 0) {
      ...
      sheetValues = [headers];
      apiLogger.info(`Added headers to ${sheetName} sheet`);
    }
