...
export default function CampaignQuizPage() {
  ...
  const triggerConversionEvents = useCallback(() => {
    if (!GOOGLE_ADS_CONVERSION_LABEL) {
      formLogger.warn(
        "[QUIZ] Google Ads conversion label not configured; skipping Ad conversion event.",
      );
    } else {
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      formLogger.info("[QUIZ] Starting form submission", {
        email: formData.email,
      ...
      formLogger.debug("[QUIZ] Submission data prepared", {
      ...
      if (sheetsResponse.status === "fulfilled") {
        ...
        formLogger.info("[QUIZ] Google Sheets response", { sheetsData });
      } else {
        formLogger.error("[QUIZ] Google Sheets error", {
      ...
      if (activeCampaignResponse.status === "fulfilled") {
        ...
        formLogger.info("[QUIZ] ActiveCampaign response", { acData });
      } else {
        formLogger.error("[QUIZ] ActiveCampaign error", {
      ...
      if (hasSuccess) {
        ...
        formLogger.info("[QUIZ] Form submitted successfully");
      ...
      formLogger.error("[QUIZ] Submission error", {
